...
import { HelpCircle, SkipForward, Users, Volume2, VolumeX } from "lucide-react";
...
export function GamePage({ players, onReturnToLobby }: GamePageProps) {
  ...
  const {
    playUnitSelect,
    playUnitMove: _playUnitMove,
    playAttack,
    playCaptureStart: _playCaptureStart,
    playCaptureComplete: _playCaptureComplete,
    playFortify,
    playMusketFire,
  ...
  useEffect(() => {
    if (selectedUnit) {
      const inForest = gameState.territories.some(
        (t) =>
          t.terrain === "forest" &&
  ...
  const handleAttack = useCallback(
    (targetId: string) => {
      ...
      handleCanvasClick(target.position.x, target.position.y);
    },
    [
      selectedUnit,
      handleCanvasClick,
  ...
  const handleExecuteAbility = useCallback(
    (abilityId: string, targetId?: string) => {
      if (!selectedUnit) return;
  ...
  const isHumanTurn = players.some(
    (p) => p.team === gameState.currentTeam && !p.isBot,
  );
  ...
  const canMove = selectedUnit && !selectedUnit.hasMoved;
  const canAttack = selectedUnit && !selectedUnit.hasActed;
  const hasReachable = reachablePositions.length > 0;
  const hasAttackable = attackableUnitIds.length > 0;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-sepia/90 to-parchment/20 flex flex-col">
      ...
      <header className="border-b-2 border-sepia/40 bg-sepia/80 px-4 py-2">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            />
            <h1 className="font-cinzel text-lg text-ochre font-bold hidden sm:block">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="text-xs font-cinzel px-2 py-1 rounded border border-parchment/20 text-parchment/70">
              {gameState.currentTeam === "NativePeoples"
                ? "🛖 Nativos"
                : "🏴‍☠️ Colonizadores"}
            </div>
          </div>
      ...
      {actionFeedback && (
        <div
          className={`text-center py-1.5 px-4 text-sm font-fell transition-all ${
            actionFeedback.type === "success"
              ? "bg-forest-green/60 text-parchment"
              : actionFeedback.type === "warning"
                ? "bg-ochre/70 text-sepia"
                : actionFeedback.type === "error"
                  ? "bg-colonizer-red/70 text-parchment"
                  : "bg-sepia/60 text-parchment"
          }`}
        >
      ...
      {isHumanTurn && selectedUnit && (
        <div className="bg-parchment/80 border-b border-sepia/30 px-4 py-1.5 flex items-center gap-4 text-xs text-sepia/80 flex-wrap">
          <span className="font-cinzel font-semibold text-sepia">
            {selectedUnit.classType} selecionado
          </span>
          {canMove && hasReachable && (
            <span className="flex items-center gap-1">
              <span className="inline-block w-3 h-3 rounded-full border-2 border-blue-400 bg-blue-400/30" />
              Clique em ponto azul para mover
            </span>
          )}
          {canAttack && hasAttackable && (
            <span className="flex items-center gap-1">
              <span className="inline-block w-3 h-3 rounded-full border-2 border-red-400 bg-red-400/30" />
              Clique em inimigo vermelho para atacar
            </span>
          )}
          {!canMove && !canAttack && (
            <span className="text-sepia/50">Unidade já agiu neste turno</span>
          )}
          <button
            type="button"
            onClick={deselectUnit}
            className="ml-auto text-sepia/50 hover:text-sepia underline text-xs font-fell"
          >
            Desselecionar (ESC)
          </button>
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2 text-center">
            {selectedUnit ? (
              <p className="font-fell italic text-xs text-ochre animate-pulse">
                {canMove && hasReachable
                  ? "🔵 Clique nos pontos azuis para mover • "
                  : ""}
                {canAttack && hasAttackable
                  ? "🔴 Clique no inimigo vermelho para atacar"
                  : ""}
                {!canMove && !canAttack
                  ? "✓ Unidade já agiu — finalize o turno ou selecione outra"
                  : ""}
              </p>
            ) : (
              <p className="font-fell italic text-xs text-parchment/50">
                Clique em uma unidade com anel brilhante para selecioná-la • ESC
                para desselecionar
              </p>
            )}
          </div>
          ...
          <div className="mt-2 flex justify-center gap-4 flex-wrap">
            <div className="flex items-center gap-1 text-xs font-fell text-parchment/50">
              <span>🌲</span>
              <span>Floresta: +15% Dano</span>
            </div>
            <div className="flex items-center gap-1 text-xs font-fell text-parchment/50">
              <span>🌾</span>
              <span>Planície: +10% Mosquete</span>
            </div>
            <div className="flex items-center gap-1 text-xs font-fell text-parchment/50">
              <span>⛰</span>
              <span>Colinas: +10% Ranged</span>
            </div>
            <div className="flex items-center gap-1 text-xs font-fell text-parchment/50">
              <span>⭐</span>
              <span>Ponto Estratégico</span>
            </div>
          </div>
        </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span
                        className={
                          isNative ? "text-native-green" : "text-colonizer-red"
                        }
                      >
                        {isNative ? "🛖" : "🏴‍☠️"} {unit.classType.slice(0, 8)}
                        {!isHuman && (
                          <span className="text-parchment/40 ml-1">(Bot)</span>
                        )}
                      </span>
                      <span className="text-parchment/60 flex items-center gap-0.5 text-xs">
                        {unit.level > 1 && (
                          <span className="text-ochre">L{unit.level}</span>
                        )}
