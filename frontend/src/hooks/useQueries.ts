...
export function useUpdateTerritoryOwnership() {
  ...
  return useMutation({
    mutationFn: async ({ gameId, territoryId }: { gameId: string; territoryId: string }) => {
      if (!actor) throw new Error('Actor not initialized');
      await actor.updateTerritoryOwnership(gameId, territoryId);
    },
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['gameSession', variables.gameId] });
    },
  });
}
...
export function useUpdateUnitPosition() {
  ...
  return useMutation({
    mutationFn: async ({ gameId, unitId }: { gameId: string; unitId: string }) => {
      if (!actor) throw new Error('Actor not initialized');
