...
export function TerritoryLegend({ gameState }: TerritoryLegendProps) {
  ...
  return (
    <div className="bg-sepia/80 border border-parchment/20 rounded-lg p-3 text-xs font-fell">
      <h3 className="font-cinzel text-xs font-bold text-ochre mb-2">🗺️ Territórios</h3>
      ...
      {/* Strategic Points */}
      <div className={`border rounded-lg p-2 mb-2 transition-all ${
        allColonized ? 'border-yellow-500/60 bg-yellow-900/20' : 'border-parchment/20 bg-parchment/5'
      }`}>
        <p className={`font-cinzel text-xs font-bold mb-1.5 ${allColonized ? 'text-yellow-400' : 'text-parchment/70'}`}>
        ...
        <div className="space-y-1">
          {strategicPoints.map(point => (
            <div key={point.id} className="flex items-center justify-between">
              <span className="text-parchment/70 truncate mr-1">{point.name}</span>
      ...
      {conquestCountdown !== null && (
        <div className="bg-yellow-900/30 border border-yellow-600/40 rounded p-2 text-center animate-pulse">
          ...
          <p className="text-yellow-300/70 text-xs mt-0.5">Todos os pontos dominados!</p>
        </div>
      )}
    </div>
  );
