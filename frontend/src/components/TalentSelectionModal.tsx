...
export function TalentSelectionModal({ unit, onSelectPath }: TalentSelectionModalProps) {
      ...
      <div className="bg-sepia border-4 border-ochre/60 rounded-xl p-6 max-w-lg w-full mx-4 shadow-2xl">
        ...
        <div className="flex items-center justify-center gap-3 mb-4">
          ...
          <div className="text-center">
            <h2 className="font-cinzel text-xl font-bold text-ochre">⬆️ Subiu de Nível!</h2>
        ...
        <div className="grid grid-cols-2 gap-4">
          {(['PathA', 'PathB'] as TalentPath[]).map(path => {
            ...
            const bonusEntries = Object.entries(bonuses).filter(([, v]) => v && v !== 0);
              ...
              >
                ...
                {bonusEntries.length > 0 && (
                  <div className="space-y-1">
                    <p className="text-xs font-cinzel text-parchment/50 uppercase tracking-wider">Bônus</p>
                    {bonusEntries.map(([stat, value]) => (
                      <div key={stat} className="flex items-center justify-between text-xs">
                        <span className="text-parchment/60 font-fell capitalize">{stat}</span>
                        <span className="text-native-green font-bold">+{value}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {pathData.abilityUpgrades.length > 0 && (
                  <div className="space-y-1">
                    <p className="text-xs font-cinzel text-parchment/50 uppercase tracking-wider">Melhoria</p>
                    {pathData.abilityUpgrades.map((upgrade, i) => (
                      <p key={i} className="text-xs text-parchment/70 font-fell">
                        ✨ {upgrade}
                      </p>
                    ))}
                  </div>
