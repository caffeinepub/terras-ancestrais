...
export function GameTimer({ timeRemaining, conquestCountdown }: GameTimerProps) {
  const isWarning = timeRemaining <= 60 && timeRemaining > 30;
  const isCritical = timeRemaining <= 30;

  const conquestIsWarning = conquestCountdown !== null && conquestCountdown <= 120 && conquestCountdown > 60;
  const conquestIsCritical = conquestCountdown !== null && conquestCountdown <= 60;

  return (
    <div className="flex flex-col gap-1 items-end">
      {/* Match Timer */}
      <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border font-cinzel font-bold text-sm transition-all ${
        isCritical
      ...
      {conquestCountdown !== null && (
        <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border font-cinzel font-bold text-sm transition-all ${
          ...
          <div className="flex flex-col items-start leading-none">
            <span className="text-xs font-fell text-black opacity-80">Conquista</span>
