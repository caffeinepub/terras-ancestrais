...
const NATIVE_CLASSES: { value: ClassType; label: string; emblem: string }[] = [
  { value: 'ForestWarrior', label: 'Guerreiro da Floresta', emblem: '/assets/generated/emblem-forest-warrior.dim_128x128.png' },
  { value: 'SpiritHunter', label: 'Caçador Espiritual', emblem: '/assets/generated/emblem-spirit-hunter.dim_128x128.png' },
  { value: 'Shaman', label: 'Xamã', emblem: '/assets/generated/emblem-shaman.dim_128x128.png' },
  { value: 'Sentinel', label: 'Sentinela', emblem: '/assets/generated/emblem-sentinel.dim_128x128.png' },
];

const COLONIZER_CLASSES: { value: ClassType; label: string; emblem: string }[] = [
  { value: 'MusketSoldier', label: 'Soldado de Mosquete', emblem: '/assets/generated/emblem-musket-soldier.dim_128x128.png' },
  { value: 'Captain', label: 'Capitão', emblem: '/assets/generated/emblem-captain.dim_128x128.png' },
  { value: 'Engineer', label: 'Engenheiro', emblem: '/assets/generated/emblem-engineer.dim_128x128.png' },
  { value: 'Missionary', label: 'Missionário', emblem: '/assets/generated/emblem-missionary.dim_128x128.png' },
];
...
export function PlayerSlotCard({ slot, onChange, slotIndex }: PlayerSlotCardProps) {
  const isNative = slot.team === 'NativePeoples';
  const classes = getClassesForTeam(slot.team);
  const selectedClass = classes.find(c => c.value === slot.class) || classes[0];
    ...
    <div className={`rounded-lg border-2 p-3 transition-all ${
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-1.5">
          <Label htmlFor={`bot-${slot.id}`} className="text-xs text-parchment/60">Bot</Label>
          <Switch
            id={`bot-${slot.id}`}
            checked={slot.isBot}
      ...
      {slot.isBot && (
        <div className="text-xs text-parchment/50 italic mb-2 px-1">🤖 Controlado por IA</div>
      )}
      ...
      <div className="space-y-1">
        ...
        <Select value={slot.class} onValueChange={(v) => handleClassChange(v as ClassType)}>
          ...
          <SelectContent className="bg-sepia border-parchment/30">
            {classes.map(cls => (
              <SelectItem key={cls.value} value={cls.value} className="text-parchment hover:bg-parchment/10">
                <div className="flex items-center gap-2">
                  <img
                    src={cls.emblem}
                    alt={cls.label}
